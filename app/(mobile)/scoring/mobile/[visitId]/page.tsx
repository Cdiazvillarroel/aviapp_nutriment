...
export default async function MobileScoringPage({
  ...
  // Try to load from server. If it fails (e.g., offline cached page),
  // render a client-only fallback that reads from IndexedDB.
  try {
    const supabase = await createClient();

    const { data: { user } } = await supabase.auth.getUser();
    if (!user) {
      return <OfflineFallback visitId={visitId} />;
    }
